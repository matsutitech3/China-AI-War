...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{15mm}p{17mm}p{28mm}p{14mm}p{58mm}}
    ...
    2026-08 & Alibaba & Qwen3.8-Max (2.4T/$\sim$95B) & committed & first Max-tier open-weight commitment; 1M context; multimodal; \$2/\$6 per Mtok (\S\ref{sec:maxtier}) \\
...
The register carries the near-term test, and this edition must report its status honestly: \emph{the committed window has slipped}. ``Next week,'' announced on 3 August, meant the week of 4--10 August; as of this edition's date no repository has appeared, and the commitment has been publicly re-framed to the week of 10 August~\cite{qwen-slip}. One week of slippage on a frontier-scale release is unremarkable as engineering and consequential as evidence, because two developments in the same week bear on \emph{what} will ship rather than whether. Reuters-sourced reporting says Alibaba is preparing revenue-sharing terms for large commercial resellers of its next open Qwen release---a Kimi-style threshold rather than Apache~2.0~\cite{qwen-revshare}. And MiniMax's H3 release demonstrated a darker variant: weights that are open \emph{except} for local deployment in the US, EU, UK, and South Korea---openness geofenced along geopolitical lines~\cite{minimax-geofence}. Neither development breaks the ratchet; both refine what it ratchets. The mechanism compels \emph{weights on the commons}, because adoption is the currency; it does not compel Apache terms or universal geography, because rent relocation and state pressure operate on exactly those margins. The forecast register (Appendix~\ref{app:dashboard}) now separates the three claims---weights ship, licence class, geographic universality---and the falsifier stands: no weights by November 2026 would break the ratchet argument as stated. The licence, when it appears, discriminates between the two versions of the rent argument rather than between openness and closure.
