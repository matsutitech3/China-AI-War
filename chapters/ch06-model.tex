...
The register carries the near-term test, and the prior on it is high: the weights are publicly committed, with a companion 27B dense model, on a stated timeline. What remains genuinely open is the \emph{licence}---Apache-class or Kimi-style revenue-threshold---and that is the question worth watching, because the answer discriminates between the two versions of the rent argument rather than between openness and closure.
