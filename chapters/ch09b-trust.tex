...
The allegation changed hands on the currency date itself. On 9~September 2026 the NSA, the FBI and CISA issued joint cybersecurity advisory AA26-251a, naming DeepSeek, Moonshot, Alibaba, MiniMax, StepFun and Z.AI as conducting distillation campaigns against US frontier models, with the traffic routed through cloud providers and third-party aggregators~\cite{cisa-distillation} [P]. Two disciplines govern how much weight that carries. The advisory concerns distillation---the harvesting of a frontier model's outputs to train another---and not hosting: it says nothing about whether the named laboratories' weights may be served, downloaded or deployed, and a reader who takes it as a finding against open-weight deployment has read a document that does not exist. And it is an allegation by intelligence and security agencies, not an adjudication by anybody; no court or tribunal has tested it, and the vendors' own February claims, which it broadly tracks, remain unadjudicated too. What is new is nonetheless real, and it is institutional rather than evidentiary. This is the first time six laboratories---very nearly the whole of the Chinese open frontier---have been named together in a single US government document, and a claim that was a vendor's grievance in February is a state's position in September. The routing detail matters more than the roster, because it lands in the same fortnight that one of the six is negotiating a revenue share with three American clouds for serving its flagship~\cite{moonshot-cloud-talks} [A]: the intermediaries through which the advisory says the distillation traffic flows are the intermediaries through which the laboratory proposes to be paid, and Section~\ref{sec:toll-road} in Chapter~\ref{ch:model} carries the argument that a contract of that kind is what makes a laboratory regulable. The advisory does not close the trust gap this chapter is about. It documents, from the other side of it, that the gap is now a matter of state record.
