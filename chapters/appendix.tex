...
\input{chapters/appendix-hbf}
\input{chapters/appendix-hotchips}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{15mm}p{15mm}p{12mm}p{11mm}p{10mm}p{16mm}p{13mm}p{15mm}p{12mm}}
    ...
    \addlinespace
    GLM-5.2 & $\sim$753B / $\sim$40B & FP8 variant & 1M var. & MIT & Ascend (press, no primary) [A/C] & 77.8\% (GLM-5, vendor) & 1.00 / 3.20 & --- \\
    \addlinespace
...
Four releases in eighteen days, each responding to the last~\cite{pareto-frontier}: \textbf{16 July}, Moonshot releases Kimi K3 (2.8T/104B) via API; \textbf{27 July}, K3's weights are open-sourced under its custom commercial licence---the largest open-weight model ever released; \textbf{30 July}, OpenAI cuts GPT-5.6 Luna---the cheapest tier---by 80\% (\$1.00$\to$\$0.20 input, \$6.00$\to$\$1.20 output) with Terra down 20\% and the flagship Sol held unchanged~\cite{openai-cut,openai-luna}; \textbf{31 July}, DeepSeek ships the official V4-Flash checkpoint ($\sim$280B/13B, MIT, weights on Hugging Face, \$0.14/\$0.28 per million tokens); \textbf{3 August}, Alibaba makes Qwen3.8-Max generally available (2.4T total, $\sim$95B active by press consensus though never officially confirmed, 1M context, multimodal in), API-only, at \$2/\$6 per million tokens~\cite{qwen38-max}.
...
\emph{The harness arrives as a competitive layer.} On 13 August DeepSeek released an MIT-licensed, plugin-native agent harness that passed 100{,}000 GitHub stars within about forty-eight hours, accompanied by an eighty-eight-page programming-languages paper from Peking University and DeepSeek, and shipped on the same day the company \emph{raised} its API prices~\cite{dsh-repo,cordis-paper,dsh-venturebeat}. Chapter~\ref{ch:harness} carries the analysis and exists because of it.

\emph{Model releases, four in five days.} Grok 4.6 (12 August, closed, 500K context, \$2/\$6 per Mtok) reached 61 on the Artificial Analysis index as it stood in launch week (v4.1.1; the index was re-based on 7~September and no score on that board is comparable to a v4.3 score~\cite{aa-v43})---tying GPT-5.6 Sol (maximum reasoning effort), two points below Claude Opus 5, and one point above the best Chinese open model---restoring xAI, now styled SpaceXAI, to the front rank; the gains come from post-training on the same 1.5T Grok 4.5 base rather than a new pretrain~\cite{grok46}. Google shipped Gemini 3.7 Flash (13 August) at \$0.75 per Mtok input~\cite{gemini37}. DeepSeek launched V4-Pro on 13 August and \emph{raised} API prices by as much as 1{,}100\%, introducing peak and off-peak tiering; Caixin described it as ``a break from the prolonged price war among China's AI developers,'' and no rationale was stated~\cite{deepseek-raise}---and the company is reported to be hiring chip-design engineers to build its own accelerator, reducing reliance on NVIDIA \emph{and} Huawei, which belongs to Chapter~\ref{ch:semi}'s ledger rather than this one~\cite{deepseek-v4pro}. Zhipu announced GLM-5.3 (14 August) claiming the strongest open-weights coding model; the weights followed on \textbf{28 August} under a custom ``glm-5.3'' licence reportedly requiring a security review of model-as-a-service operators above \$10B of trailing revenue---so the claim is now checkable, and on the two headline agentic-coding suites the model still \emph{trails} GPT-5.6 Sol~\cite{glm53}. Nine days after the announcement Zhipu released \textbf{GLM-5.3-Flash} (26 August, MIT, 320B total and 18B active), revealed on 27 August to have been the stealth model ``Ox Alpha'' that had led the router boards all week, and stated that the preview had run ``entirely on Chinese AI chips''~\cite{glm53-flash,glm53-chips}. Section~\ref{sec:glm53} grades the serving claim and its three qualifications---served is not trained, no chipmaker is named, and it is not a first.

\emph{A social platform ships an agent model (14 August).} Xiaohongshu's dots studio released \emph{dots3-note Preview} under Apache 2.0---280B total, 16B active, text, image, video and audio input, 512K context, weights on Hugging Face and ModelScope, free on OpenRouter---with a reinforcement-learning method, TEMPO, aimed at agent trajectories of ten hours and more, and self-reported SWE-bench Verified 78.4 and SWE-bench Pro 61~\cite{dots3-note} [V for the release and licence; P for the benchmarks]. Huawei's Ascend ecosystem published day-zero vLLM-Ascend deployment on Atlas 800 A3 and Atlas 900 A3 within two days~\cite{dots3-ascend} [A]. Two disciplines apply: the model is labelled a preview and its technical report is ``coming soon,'' so the method is described from the release material and graded accordingly; and Vercel's July gateway report---open-weight models at 29\% of enterprise gateway tokens on under 4\% of spend in June~\cite{vercel-gateway} [V]---is entered here as the cleanest measurement yet of the volume--revenue split that Chapter~\ref{ch:trade} argues over. Section~\ref{sec:model-survey} and Chapter~\ref{ch:harness} carry the analysis: what the release records is that frontier-scale, agent-trained open models are now something a Chinese consumer platform is expected to ship.

\emph{NVIDIA and the open commons: a report, not an announcement.} \emph{The Information} reported on 12 August that NVIDIA is in the final training phase of a trillion-parameter ``Nemotron 4'' intended to compete with the world's most powerful \emph{open} models, with possible availability in late autumn~\cite{nemotron4}. NVIDIA's own position, as quoted, is that final training is not complete; no licence has been announced; and the company's best shipped open model today scores 38 on the independent index against Kimi K3's 60, both on the August (v4.1.1) board; the September re-basing moved every score and the two figures are comparable only with each other~\cite{aa-v43}. The event, correctly stated, is that the largest beneficiary of the closed-frontier trade is reported to be preparing an open frontier-scale release---which, if it ships, is a second American entry into the commons after Meta's and a further datapoint for the ratchet of Section~\ref{sec:maxtier}. It has not shipped.

\section{The financing turn: the same week, in capital markets}

Three developments in the week of 10--14 August\ belong to Chapter~\ref{ch:trade} rather than to the model chronology, and together they describe the trade acquiring market infrastructure.

\emph{NVIDIA mobilizes third-party capital (10 August).} Memoranda of understanding with Apollo, BlackRock, Blackstone, Brookfield, Goldman Sachs and KKR to establish compute financing platforms targeting over \$500B of third-party capital ``over time,'' subject to final agreements; the subhead's object is turning NVIDIA compute into ``an investable asset class''~\cite{nvda-500b}. Four days later, reporting had NVIDIA cutting its contemplated backstop of OpenAI's Ohio campus from roughly \$250B to under \$120B after investors raised concerns about its risk exposure~\cite{nvidia-backstop-cut}. Read together: exposure relocating from the vendor's balance sheet to institutional capital.

\emph{OpenAI's commercial leadership turns over before a listing (11--13 August).} The longest-tenured business executive and the first chief revenue officer departed within four days, with the finance function conspicuously intact---the CFO briefed investors on 14 August, reporting an annualized run rate near \$40B and enterprise revenue overtaking consumer~\cite{openai-exits}. No listing date is set; prediction markets priced a 2026 listing near 19\%.

\emph{Compute becomes an exchange-traded product (11 August).} CME and Silicon Data announced compute futures for 5 October, listed on NYMEX---compute filed as an energy derivative---joining ICE's two announced families, one of them energy-normalized with a dedicated inference sub-index, and Kalshi's live GPU-price contracts~\cite{cme-compute,ice-nativx,kalshi-compute}. Section~\ref{sec:commodity} carries the analysis and works through what a traded, benchmarked, backwardated market for compute does to pricing power---and why the Chinese lever turns out to be the model layer rather than the GPU-hour.

\section{The memory question, entered into the record}

Two developments concern memory rather than models, and both are conditional claims that the register now carries.

The Open Compute Project published the first High Bandwidth Flash specification on 3~August 2026---512\,GB per stack, 0.4 to 3.0\,TB/s, UCIe rather than PCIe, with SanDisk, SK~hynix, Google and Tenstorrent as members and NVIDIA, Samsung, Micron and Kioxia absent~\cite{hbf-ocp,hbf-spec}. Nothing is sampling; no endurance, latency, power or price figure has been published. Section~\ref{sec:memoryunlock} works through what a terabyte-class weights tier paired with an LPDDR6X context tier would do to the economics of local inference. Non-volatility is the term that matters: a weights tier that draws nothing at rest, at a realistic duty cycle, reaches energy parity with continuously refreshing DRAM, and the residual difference is tens of dollars a month against a metered service costing thousands. The question turns on HBF shipping at its specified capacity and bandwidth, not on its energy figure. Two facts frame the conditional in opposite directions---NAND is forecast into surplus in 2H27 while DRAM stays scarce, which is a durable relative-price tailwind; and NVIDIA removed its capacity-first inference part from the roadmap while paying a reported \$20B for the opposite architecture, which is the best-informed buyer in the industry betting against capacity.

Alongside it, the price record for this edition: conventional DRAM contract prices compounded roughly 3.6$\times$ across the first three quarters of 2026, and Apple withdrew the 512\,GB Mac Studio configuration in March---the flagship consumer machine for local inference on large open models becoming \emph{less} capable because of memory scarcity, in the same year the commons reached terabyte scale~\cite{trendforce-dram,mac-studio-512}.

\section{The Pareto observation}

The single most strategically significant datum of the barrage is not any model's score but the shape of the cost-performance frontier it produced: of the five models on the current Pareto frontier, \emph{four are Chinese}---Kimi K3, Qwen3.8-Max, GLM-5.3-Flash, and DeepSeek V4-Flash---with Claude Opus~5 the lone American entry, holding its position by roughly 37 Elo points~\cite{pareto-frontier}. The frontier as originally computed put GLM-5.2 in that fourth place; the late-August releases of Section~\ref{sec:glm53} displace it, because GLM-5.3-Flash dominates it on both axes---index 57 at \$0.09 per task on the launch-week v4.1.1 board; 42 at \$0.25 per task on the re-based v4.3 board of 7~September, the two pairs not being comparable with each other~\cite{aa-v43,glm53-flash-agentic}---so the substitution strengthens the observation rather than weakening it. Read against the five-frontier framework of Section~\ref{sec:scoreboard}, this is the \emph{economic frontier} passing decisively into Chinese hands while the \emph{absolute frontier} remains contested, which is precisely the configuration this book has argued decides industrial outcomes. The cost spread that produces it: DeepSeek V4-Flash runs a benchmark suite for about three cents against Claude Fable~5's \$3.15 and GPT-5.6 Sol's \$1.86---a hundredfold gap at the low end, with Kimi K3 at 86 cents in between.
...
One further pricing development belongs in the ledger because it runs \emph{against} the commoditization current, and it arrived in two dated steps rather than one. First, with the official V4-Flash release of 31~July 2026, DeepSeek announced dynamic peak pricing---a 2$\times$ multiplier across seven daily hours---on the cheapest frontier-adjacent model in the world~\cite{v4flash}. Then, on 13~August 2026, the company launched V4-Pro and \emph{raised} API prices by as much as 1{,}100\%, introducing peak and off-peak tiering across the line; Caixin described it as ``a break from the prolonged price war among China's AI developers,'' and no rationale was stated~\cite{deepseek-raise}. A Chinese laboratory introducing surge pricing and then repricing upward is rent recapture appearing at the price layer itself, and Section~\ref{sec:china-advice}'s recommendation that Chinese labs move from below-cost tokens to at-cost tokens plus paid complements now has a datapoint in its favour that arrived without waiting for the advice.
...
\emph{Newly falsifiable, on a short clock}: the Qwen3.8-Max licence question has since resolved (a revenue-threshold licence, recorded above); still open is whether any AI-assisted design flow produces an independently verified, manufacturable part. Few strategic questions in this book resolve within a quarter. The first of these does.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{16mm}p{134mm}}
    ...
    \addlinespace
    \textbf{2025} & R1 released (Jan.); NVIDIA loses \$589B in a day. Eight-agency RISC-V guidance reported in preparation (Mar.; never confirmed published) [A]. H20 banned (Apr.), unbanned for a 15\% revenue share (Aug.). Kimi K2 opens at 1T parameters (Jul.). WAIC Global AI Governance Action Plan and ``AI+'' plan (Jul.--Aug.). CANN open-sourced (Aug.); CUDA ported to RISC-V hosts, announced in Shanghai (Jul.). Huawei discloses the Ascend 950/960/970 roadmap and opens UnifiedBus 2.0 (Sep.). CAC bars major platforms from NVIDIA purchases (Sep.); state-funded data centres restricted to domestic chips and given up-to-50\% power discounts (Nov.). K2 Thinking ships INT4 QAT (Nov.). China adds $\sim$540\,GW of generation; deploys $\sim$65\% of global grid storage. \\
    \addlinespace
    \textbf{2026 H1} & Zhipu and MiniMax list in Hong Kong (Jan.). SiFive integrates NVLink Fusion (Jan.). Trade press reports GLM-5 trained on $\sim$100k Ascend 910B (Jun.; no locatable primary) [A/C]; Anthropic alleges distillation campaigns (Feb.). Stanford AI Index puts the US--China gap at 2.7\% (Mar.). NVIDIA Vera CPU announced at 1.2\,TB/s LPDDR5X (Mar.). DeepSeek V4 (Apr.); OpenAI raises at \$852B (Mar.); Anthropic at \$965B (May). Semiconductor selloff sheds \$1.3T (Jun.). \textbf{LineShine takes No.~1 on HPL and HPCG with no GPUs} (Jun. 24). \\
    \addlinespace
    ...
    \addlinespace
    \textbf{2026 Aug--Sep} & Brazil contracts Huawei/iFlytek for a Rio Ascend machine, R\$1.3B, self-financed, inside a split programme (Aug. 21). Moonshot--Microsoft/Amazon/Google revenue-share talks on Kimi K3 reported, up to 30\%, unsigned (Aug. 26); Huawei bid for Egyptian government AI datacentres on Ascend 950 reported, US counter-offer assembled (Aug. 26) [A]. Commerce rule to block Chinese rental of NVIDIA compute in third countries reported in draft (Aug. 28) [A]. Xi in Cairo; no AI agreement (Aug. 31--Sep. 2). Moonshot confidential Hong Kong filing reported, $\sim$\$3B at $\sim$\$50B (Sep. 3) [A]. DeepSeek order of $\geq$160{,}000 Ascend 950DT for an Ulanqab inference site reported; training stays on NVIDIA (Sep. 4) [A]. Telekom Malaysia evaluating Ascend 910C for a RM2B sovereign project reported, unsigned (Sep. 7) [A]; Artificial Analysis re-bases to v4.3 (Sep. 7). NSA/FBI/CISA advisory AA26-251a names six Chinese laboratories for distillation (Sep. 9). \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{small}
\begin{longtable}{p{42mm}p{16mm}p{13mm}p{31mm}p{31mm}}
  ...
  \midrule
  \endfirsthead
  \multicolumn{5}{l}{\footnotesize\emph{Forecasts, continued}} \\
  \toprule
  Proposition & Probability & Horizon & Next verification event & Explicit falsifier \\
  \midrule
  \endhead
  \midrule
  \multicolumn{5}{r}{\footnotesize\emph{continues}} \\
  \endfoot
  \bottomrule
  \endlastfoot
    ...
    \addlinespace
    Qwen3.8-Max weights ship as publicly committed & \textbf{RESOLVED YES} (13 Aug 2026) & --- & shipped to Hugging Face; product name not used on the repository~\cite{qwen38-weights} & --- \\
    \addlinespace
    ...
    \addlinespace
    A Chinese-origin agent harness or agent runtime reaches top-three position by production deployment in at least one major non-Chinese market (Chapter~\ref{ch:harness}) & 35\% & 2--4 yrs & enterprise deployment surveys; package-registry download share, not stars & Western harnesses holding $>$90\% of regulated-sector production deployments through 2029 \\
    \addlinespace
    ...
    \addlinespace
    Exchange-traded compute futures reach sustained liquidity---continuous two-sided markets and open interest disclosed by the venue---rather than remaining reference benchmarks (\S\ref{sec:commodity}) & 40\% & 2--3 yrs & CME, ICE and Kalshi volume and open-interest reporting after the Oct.\ 2026 launch & thin or discontinuous trading through 2028, leaving the indices as pricing references only \\
    \addlinespace
    A dedicated \emph{inference} contract (COIL-I class) trades independently of GPU-rental contracts, establishing tokens rather than GPU-hours as the traded unit & 30\% & 2--4 yrs & venue product listings and settlement data & inference pricing remaining bundled into GPU-hour or per-vendor API terms \\
    \addlinespace
    Chinese inference capacity is exported at scale---token-denominated services to non-Chinese buyers materially visible in services-trade statistics or neutral-router data (\S\ref{sec:commodity}) & 45\% & 3--5 yrs & customs/services-trade series; neutral-router provider shares; enterprise procurement surveys & Chinese endpoint share stalling in Western regulated sectors while open-weight self-hosting carries the volume instead \\
    \addlinespace
    HBF-class silicon ships in volume at its specified capacity and bandwidth ($\geq$512\,GB per stack, $\geq$1.6\,TB/s) with a published endurance figure adequate for a weights tier (\S\ref{sec:memoryunlock}) & 45\% & 2--4 yrs & vendor datasheets; independent measurement; OCP conformance scheme & no volume product by 2029, or shipped bandwidth below $\sim$0.8\,TB/s per stack, which would leave a 2.8T-parameter model below reading speed for a single user \\
    \addlinespace
    A capacity-first appliance runs a $\geq$2T-parameter open model for four concurrent users at $\geq$40 tokens/s each inside a 1\,kW envelope, at a system price below \$8k (\S\ref{sec:appliance-cost}) & 35\% & 3--5 yrs & published measurements on shipping hardware & the configuration remaining above \$15k, or below 20 tok/s per user at four users, through 2030 \\
    \addlinespace
    ...
    \addlinespace
    A hyperscale-class AI facility ($\geq$100\,MW) is sold, foreclosed or written down at $\geq$40\% below cost before 2030---the receivership case of Chapter~\ref{ch:trade} & 45\% & 2--4 yrs & REIT and neocloud disclosures; distressed-asset transactions; lender filings & every 2025--26 vintage refinancing at par through 2029, which would mean the facility utilization forecasts held \\
    \addlinespace
    ...
    \addlinespace
    Merchant-GPU data-centre revenue growth decelerates while merchant custom-ASIC revenue continues at Broadcom's guided trajectory---the silicon-commoditization pincer of Chapter~\ref{ch:mirror} & 50\% & to end-2027 & quarterly segment disclosures from NVIDIA and Broadcom~\cite{broadcom-q2fy26} & merchant-GPU data-centre growth holding while custom-ASIC growth decelerates, which would leave the pincer unobserved \\
    \addlinespace
    Custom accelerators continue converging onto NVIDIA's interconnect and memory rather than displacing them---the absorption reading of Chapter~\ref{ch:mirror} & 65\% & to end-2027 & accelerator announcements and teardowns; Trainium4 on NVLink Fusion and NVIDIA custom HBM~\cite{aws-nvda} & a major custom program shipping a scale-up fabric and memory stack independent of NVIDIA's \\
    \addlinespace
    First confirmed Ascend export outside China (Egypt or Malaysia; Brazil's self-financed Rio machine, due July 2027, is excluded as already contracted)---resolves the telecom-template question of Chapter~\ref{ch:telecom} and \S\ref{sec:export-offer} & 40\% [25\%] & to end-2027 & a signed, disclosed contract and a delivered installation; press reporting confirmed by the recipient government~\cite{huawei-egypt,huawei-malaysia} & no signed Ascend contract in Egypt or Malaysia by end-2027, or both projects awarded to US vendors, which would leave the export offer at one self-financed contract in Brazil \\
\end{longtable}
\end{small}

\section{Quarterly falsification dashboard}

Fifteen indicators, each with the value as of \datafreeze, the threshold that would move the argument, and where to look. A reader who checks these quarterly does not need to re-read the book to know whether it is aging well.

\begin{small}
\begin{longtable}{p{40mm}p{24mm}p{34mm}p{40mm}}
  \caption{The dashboard. ``Direction'' states which way a move cuts for this book's thesis.}
  \label{tab:dashboard} \\
  \toprule
  Indicator & Current (Sep 2026) & Threshold that matters & Source / cadence \\
  \midrule
  \endfirsthead
  \multicolumn{4}{l}{\footnotesize\emph{Dashboard, continued}} \\
  \toprule
  Indicator & Current (Sep 2026) & Threshold that matters & Source / cadence \\
  \midrule
  \endhead
  \midrule
  \multicolumn{4}{r}{\footnotesize\emph{continues}} \\
  \endfoot
  \bottomrule
  \endlastfoot
    ...
    \addlinespace
    Wholesale colocation rent, NoVA 10\,MW+ (\$/kW-month, power passed through) & \$155--185; vacancy 0.5\% & a sustained decline with rising vacancy is the facility-side signal that the utilization forecast of \S\ref{sec:appliance-cost} is failing; a rise says the closet is not yet displacing the hall & CBRE / JLL market reports, semiannual \\
    \addlinespace
    ...
    \addlinespace
    Max-tier openness lag (API-to-weights interval for the leading Chinese flagship) & $\sim$10 days realized (Qwen3.8-Max); 14 days (GLM-5.3) & a lag exceeding two quarters, or any frontier tier staying closed, would break the ratchet of \S\ref{sec:maxtier} & model repositories, per release \\
    \addlinespace
    ...
    \addlinespace
    Offshore compute leasing by Chinese labs & permitted; reportedly in use; Commerce rule to close it reported in draft, unpublished, authority contested (28 Aug.)~\cite{remote-access-rule} & closure by rule would be the highest-impact tightening available to export policy & BIS/Commerce rulemaking \\
    \addlinespace
    First confirmed Ascend export outside China & none; one contract (Brazil, delivery Jul.\ 2027), one bid (Egypt), one evaluation (Malaysia), all unfinanced by the vendor~\cite{huawei-egypt,brazil-ai-supercomputer} & a delivered, confirmed Ascend installation abroad on Chinese credit would move the telecom template of Chapter~\ref{ch:telecom} from intent to execution & Bloomberg/Reuters; recipient-government disclosures; BIS enforcement \\
\end{longtable}
\end{small}
