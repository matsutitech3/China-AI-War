...
\chapter{Current Cycle: The Barrage Goes Bipolar}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{42mm}p{16mm}p{13mm}p{31mm}p{31mm}}
    ...
    \addlinespace
    Qwen3.8-Max weights ship as publicly committed & 90\% \emph{(held open: the committed week lapsed; window re-framed to 10 Aug~\cite{qwen-slip})} & 1 quarter & Hugging Face / ModelScope repository & no weights by Nov 2026 (would break the ratchet argument of \S\ref{sec:maxtier}) \\
    \addlinespace
    \dots{} under a permissive (Apache/MIT-class) rather than revenue-threshold licence & 55\% $\to$ 30\% \emph{(revised 11 Aug on the revenue-share reporting~\cite{qwen-revshare})} & 1 quarter & the licence file & a Kimi-style threshold, which confirms rent relocation \emph{within} openness rather than against it \\
    \addlinespace
