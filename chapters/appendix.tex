...
\chapter{Current Cycle: The Eighteen-Day Barrage}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{34mm}p{58mm}p{50mm}}
    ...
    \textbf{Weights} & \textbf{publicly committed by Alibaba: ``next week, the open weights of Qwen3.8-Max will be released, and Qwen3.8-27B is also going open-weights''} & \textbf{[V] as a commitment}; repository pending at the bulletin date~\cite{qwen-announcement} \\
    Licence & --- & \emph{unstated}---the live question (Apache-class vs revenue-threshold) \\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{42mm}p{16mm}p{13mm}p{31mm}p{31mm}}
    ...
    \addlinespace
    Qwen3.8-Max weights ship as publicly committed & 90\% & 1 quarter & Hugging Face / ModelScope repository & no weights by Nov 2026 (would break the ratchet argument of \S\ref{sec:maxtier}) \\
    \addlinespace
    \dots{} under a permissive (Apache/MIT-class) rather than revenue-threshold licence & 55\% & 1 quarter & the licence file & a Kimi-style threshold, which confirms rent relocation \emph{within} openness rather than against it \\
    \addlinespace
